...
type WalletService struct {
    *cantonvalidator.CantonGRPCClient
}
...
func (c *WalletService) CreateParty(ctx context.Context, walletName string) (string, error) {
	adminToken, err := cantonvalidator.GenerateToken(cfg, cfg.LedgerAPIAdminUser)
	if err != nil {
...
func (c *WalletService) CreateUser(ctx context.Context, userID string, partyID string) error {
	adminToken, err := cantonvalidator.GenerateToken(cfg, cfg.LedgerAPIAdminUser)
	if err != nil {
		return fmt.Errorf("failed to get admin token: %w", err)
	}
...
func (c *WalletService) OnboardWallet(ctx context.Context, email string, password string) (string, error) {
	token, err := cantonvalidator.GetUserToken(cfg, email, password)
	if err != nil {
...
func (c *WalletService) PreApproveTransfers(ctx context.Context, email string, password string) error {
	token, err := cantonvalidator.GetUserToken(cfg, email, password)
	if err != nil {
		return fmt.Errorf("failed to get user token: %w", err)
	}
...
func (c *WalletService) CreateWallet(
	ctx context.Context,
	walletName string,
	email string,
	password string,
	...
	mgmtToken, err := cantonvalidator.FetchAuth0ManagementToken(cfg)
	if err != nil {
		return nil, fmt.Errorf("failed to get management token: %w", err)
	}
	auth0UserID, err := cantonvalidator.CreateAuth0User(cfg, mgmtToken, email, password)
	if err != nil {
...
func (c *WalletService) CreateAndSetupWallet(ctx context.Context, walletName string, email string, password string) (partyID, userID string, err error) {
